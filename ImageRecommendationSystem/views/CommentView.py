...
#############PC接口
class  Comment(PublicView.EntryView):
    def __init__(self):    
        entryModel = CommentModel.Comment
        entryObject =  CommentModel.Comment
        entryName = "Comment"
        entryNameChinese = "评论"
    ...
    def index_query(self,request,query):
        return  CommentModel.Comment.objects.values().filter(Q( __contains=query))
